...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        toggles.forEach(toggle => {
            toggle.addEventListener('change', function (event) {
                ...
                data.append('enabled', isEnabled ? '1' : '0');
                ...
                fetch(ajaxurl, {
                    ...
                    .then(result => {
                        if (result.success) {
                            // Reload the page to reflect changes, like the default provider list.
                            window.location.reload();
                        } else {
                            this.checked = !isEnabled;
                            alert(result.data.message || '<?php _e('An error occurred. Please try again.', 'two-factor-login-telegram'); ?>');
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        this.checked = !isEnabled;
                        alert('<?php _e('A network error occurred. Please try again.', 'two-factor-login-telegram'); ?>');
                    })
                    .finally(() => {
                        this.disabled = false;
                    });
    ...
    function regenerateRecoveryCodes() {
        const userId = document.getElementById('regenerate_recovery_codes_btn').getAttribute('data-user-id');
        const nonce = document.getElementById('regenerate_recovery_nonce').value;

        const data = new URLSearchParams();
        data.append('action', 'tg_regenerate_recovery_codes');
        data.append('user_id', userId);
        data.append('nonce', nonce);
        ...
        fetch(ajaxurl, {
            ...
            .then(data => {
                if (data.success) {
                    const codesHtml = data.data.codes.map(code => `<li>${code}</li>`).join('');
                    const recoveryCodesContainer = document.querySelector('.authpress-recovery-codes');
                    recoveryCodesContainer.innerHTML = `
                    <p><strong><?php _e('Your new recovery codes are:', 'two-factor-login-telegram'); ?></strong></p>
                    <ul class="authpress-recovery-codes-list">${codesHtml}</ul>
                    <p><em><?php _e('Please save these codes in a safe place. They will not be shown again.', 'two-factor-login-telegram'); ?></em></p>
                `;
                } else {
                    alert(data.data.message || '<?php _e('An error occurred. Please try again.', 'two-factor-login-telegram'); ?>');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                alert('<?php _e('An error occurred. Please try again.', 'two-factor-login-telegram'); ?>');
            });
    }
</script>